...
$.fn.videoCt = function (options) {
    ...
    var $videoCt_img = $('.video-img', $video_container);
    ...
    var getVideoInf = function (){
        ...
        if ($videoCt[0].readyState != 4 && $('.video-seek video')[0].readyState != 4) {
           ...
           /* $('.video-img').fadeOut();*/
            ...
            $videoCt_title.find('span').text(intOpts.title);
			//图片
			if (!intOpts.img) {
                intOpts.img = opts.img;
            }
            $videoCt_img.find('img').attr('src',intOpts.img);
            //载入弹幕
			...
			//标记
			$('.video-sign').off('click').on('click',function(){
					exitFullscreen();
                    $('body').removeClass('fullScreen');
                    intOpts.fullScreen = false;
					$('.video-playw').removeClass('active');
					$('.descript-video').show();
					$videoCt[0].pause();
                    intOpts.status = false;
                    comStop();
                    $('#signText').val('');
                    $('#signTime').text(gTimeFormat($videoCt[0].currentTime));
                    $('.video-signBox').fadeIn();
                    $('.video-signBox').find('.sign-submit').show();
                    $('.video-signBox').find('.sign-look').hide();
                    $('.video-signBox').find('.prevnext').hide();
                    $('.signtitle').find('.caozuo').removeClass('delete').addClass('shezhi');
			});
			//标记弹框关闭
			$('.video-signBox .close').off('click').on('click', function () {
                $('.video-signBox').fadeOut();
                $('.signbg').removeClass('cur');
            });
            //确定标记
            $('.video-signBox .sign-submit').off('click').on('click', function () {
            	var vprpo = $videoCt[0].currentTime/$videoCt[0].duration;
            	var signtext = $('#signText').val();
            	var allSign = '<div class="allSign">'+
                            '<div class="signBox">'+
                            '<div class="signclose">'+
                            	'<span>全部标记</span>'+
                            	//'<i class="close">×</i>'+
                       ...
                       '</div>';
				if( vprpo < 1){
                    var tl = vprpo * ($videoCt_seek.width() - 8);
                    $('.setSign').append($('<p class="signbg" onclick=showSign(this) attr="'+ vprpo +'msg" style="left:'+ tl +'px"></p>'));
				}
            	if(signtext!=''){
            		$('.video-signBox').fadeOut();
            		$('#signTime').text(gTimeFormat($videoCt[0].currentTime));
	                 ...
	                 intOpts.status = true;
	                 $('#signText').val('');
	                 if($("#putvideo").find('.allSign').length<1){
	                 	$('#putvideo').append(allSign);
	                 }
	                 var singleSign = '<div class="signList" id="'+ vprpo +'msg">'+ signtext +'</div>';
	                 $('#sign-List').append(singleSign);
            ...
            $videoCt_screen.off('click').on('click', function () {
                if (intOpts.fullScreen == true) {
                    ...
                    $('body').removeClass('fullScreen');
                    intOpts.fullScreen = false;
                     $('.video-title').find('a').show();
                } else {
                    requestFullScreen();
                    $('body').addClass('fullScreen');
                    intOpts.fullScreen = true;
                    $('.video-title').find('a').hide();
                }
                $('.seek-slider').css('left', intOpts.currentTime / intOpts.duration * $('.video-seek').width() - 8);
                $('.seek-con').css('width', intOpts.currentTime / intOpts.duration * $('.video-seek').width());
            })
    ...
    function videoDom( opts ) {
        var title = $('<!--标题--><div class="video-title"><span></span><a onclick="inforSlide()">课程基本信息<em></em></a></div>');
        var img = $('<!--缩略图--><div class="video-img"><img src="" /></div>');
        var sign = $('<!--标记--><div class="video-sign">打点</div>');
        ...
        var signBox = $('<!--标记box-->\
                       <form class="signform">\
                            <div class="video-signBox">\
                            <div class="signtitle">\
                            	<span>打点位置:</span>\
                            	<em id="signTime"></em>\
                            	<em class="caozuo shezhi"></em>\
                            </div>\
                            <div class="sign-content">\
                            <div class="text"><textarea id="signText" placeHolder="输入标记"></textarea></div>\
                            <input type="button" class="sign-submit" value="">\
                            <input type="button" class="sign-look" onclick="saveSign(this)" value="">\
                            <input type="button" class="sign-cancel close" value="">\
                            <div class="prevnext"><a class="prev" onclick="prev(this)"></a><a class="next" onclick="next(this)"></a></div>\
                            </div>\
                            </div>\
                       </form>');
        ...
        if( opts.sign ){sign.appendTo($(".video-controls"))}
        ...
        if(opts.signBox){
        	if($('body').find('.signform').length==0){
        		$('#putvideo').append(signBox)
        	}
        }
    }
